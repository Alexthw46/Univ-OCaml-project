...
let exp_to_type (e : exp) : typeSet = 
	match e with
		EtypeSet(Int) -> (Int:typeSet)|
		EtypeSet(Bool) -> (Bool:typeSet)|
		EtypeSet(String) -> (String:typeSet)|
...
let rec eval (e : exp) (r : evT env) : evT = match e with
   ...
   Union(s1, s2) -> setUnion (eval s1 r) (eval s2 r)| 
   Intersection(s1, s2)-> setInts (eval s1 r) (eval s2 r)| 
   Difference (s1, s2)-> setDiff (eval s1 r) (eval s2 r)|
   Add(t, set) -> addSet (eval t r) (eval set r) |
   Remove(t, set) -> removeSet (eval t r) (eval set r)|
   Include (s1, s2) -> includeSet (eval s1 r) (eval s2 r)| 
   Head s -> head (eval s r)| 
   Length s -> length (eval s r)| 
   GetMax s -> let m = getSetMax (eval s r) in
   		(match m with
   			Some(t) -> t |
   			None -> failwith ("Empty set"))| 
   GetMin s -> let m = getSetMin (eval s r) in
   		(match m with
   			Some(t) -> t |
   			None -> failwith ("Empty set"))| 
   IsEmpty s -> isEmptySet (eval s r)|
   ForAll (p, s) -> testAll p (eval s r)| 
   Exists (p, s) -> existSet p (eval s r)| 
   Filter (p, s) -> filterSet p (eval s r)|
	Map (f, s) -> mapSet f (eval s r)
